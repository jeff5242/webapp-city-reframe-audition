...
1. **Extractors** (`auditor/extractors/`) — parse raw data from PDFs:
   ...
   - `parsers/vlm_reader.py` — the Tier 0 VLM client. Renders the review-table page (size-capped to 2000px), POSTs to an OpenAI-compatible on-prem endpoint (`/v1/chat/completions`, e.g. Qwen2.5-VL via vLLM/LLaMA-Factory), maps the returned JSON to `ReviewTableData` fields. Also `transcribe_page()` for generic whole-page VLM OCR. Sovereign (data stays on-prem); degrades to `{}`/`None` when unset or on any error.
   - `parsers/page_text.py` — per-page text provider that routes each page to the right engine: native text layer when present (fast), else on-prem VLM transcription (only when `VLM_ENDPOINT` set — never bulk CPU OCR, so text/scanned-interleaved docs stay fast). `pages_text()` is a drop-in for `extract_pages_text` used by `front_docs`/`term_checker` so scanned pages are no longer silently skipped.
